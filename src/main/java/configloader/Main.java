...
import configloader.data.GameConfig;
import configloader.data.UserInterfaceConfig;
...
public class Main {
  public static final Path GAME_CONFIG_PATH = Path.of("resource/game-properties.cfg");
  public static final Path UI_INTERFACE = Path.of("resource/user-interface.cfg");
  ...
  public static void main(String[] args)
    ...
    //UserInterfaceConfig userInterfaceConfig = createConfigObject(UserInterfaceConfig.class, UI_INTERFACE);
  ...
  public static <T> T createConfigObject(Class<T> clazz, Path filePath)
    ...
    while (scanner.hasNextLine()) {
      ...
      try {
        ...
        System.out.println(String.format("Property name : %s unsupported", propertyName));
        continue;
      }

      field.setAccessible(true);

      Object paredValue = parseValue(field.getType(), propertyValue);
      field.set(configInstance, paredValue);
    }
