...
public class Main {
  ...
  public static void main(String[] args) throws IllegalAccessException {
    Company company = new Company("Udemy", "San Francisco", new Address("Harrion Street",
        (short) 600));
    Address address = new Address("Main Street", (short)20);
    Person person = new Person("John", true, 29, 100.455f, address, company);
  ...
  private static String objectToJson(Object instance, int indentSize) throws IllegalAccessException {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      stringBuilder.append(formatToString(field.getName()));
      stringBuilder.append(":");

      if (field.getType().isPrimitive()) {
        stringBuilder.append(formatPrimitiveValue(field, instance));
      } else if (field.getType().equals(String.class)) {
        stringBuilder.append(formatToString(field.get(instance).toString()));
      } else {
  ...
  private static String formatPrimitiveValue(Field field, Object parentInstance)
    ...
    throw new RuntimeException(
        String.format("Type : %s is unsupported!", field.getType().getName()));
  }

  private static String formatToString(String value) {
